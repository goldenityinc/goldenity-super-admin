...
function getSolutionIcon(solutionCode: string) {
  if (solutionCode === SCHOOL_ERP_SOLUTION_CODE) {
    return School;
  }

  if (solutionCode === ERP_SOLUTION_CODE) {
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
          <label className="space-y-1">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-md border border-slate-300 py-2 pl-9 pr-3 outline-none ring-primary/30 focus:ring"
              />
            </div>
          </label>
          ...
          <div className="space-y-1">
            <span className="flex items-center gap-1 text-sm font-medium text-slate-700">
              <Blocks size={14} className="text-slate-500" />
              Filter Produk
            </span>
            <div className="flex min-h-[42px] flex-wrap items-center gap-2 rounded-md border border-slate-300 px-3 py-2">
              {availableSolutionFilters.length === 0 ? (
                ...
                availableSolutionFilters.map((solution) => {
                  ...
                  const SolutionIcon = getSolutionIcon(solution.code);
                  ...
                  return (
                    ...
                    >
                      <SolutionIcon size={12} className="mr-1 inline-block" />
                      {solution.code}
                    </button>
                  );
                })
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      ...
                      {row.instances.map((instance) => {
                        const SolutionIcon = getSolutionIcon(instance.solution.code);
                        return (
                          <span
                            key={instance.id}
                            className={[
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
                                  <span className="inline-flex items-center gap-1">
                                    <Clock3 size={12} />
                                    Sisa: {formatRemaining(instance.endDate ?? null)}
                                  </span>
                                  <span className="inline-flex items-center gap-1">
                                    <CalendarDays size={12} />
                                    End: {formatEndDate(instance.endDate ?? null)}
                                  </span>
                                </div>

                                <div className="space-y-1">
                                  <div className="inline-flex items-center gap-1 text-xs font-medium uppercase tracking-wide text-slate-500">
                                    <Link2 size={12} />
                                    URL Login
                                  </div>
